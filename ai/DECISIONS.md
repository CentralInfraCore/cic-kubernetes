...
primitíva-grammatika tervezési háttere, nem a `cic-kubernetes` saját döntései. A
`primitive.txt` hivatkozott részletes thread **nem létezik ebben a repóban**
(a `cic-primitives`-ban él).

A `cic-kubernetes` egyelőre nem hozott saját, kubernetes-specifikus tervezési döntést —
