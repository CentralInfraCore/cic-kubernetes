...
A `cic-kubernetes` egy **domain-repó** a `cic-primitives` (a CentralInfraCore
meta-séma rétege) fölött — kubernetes objektumokat lenne hivatva leírni.
...
kubernetes-specifikus domain compositionja sem.
...
base-repo
  └─[remote: base]─► cic-primitives
                          └─[remote: base]─► cic-kubernetes (ez a repo), cic-network, ...
```
...
|---|---|---|
| `cic-primitives` | git remote `base` | upstream → cic-kubernetes (ez a repo) |
...
repó ezt öröklés útján kapta meg, nem maga hajtotta végre. A `cic-kubernetes` saját,
valódi státusza: a release pipeline lefutott (git tag-ek tanúsítják), de
**kubernetes-specifikus domain composition egyelőre nincs megírva** — lásd a
...
## Release folyamat (a `cic-primitives`-tól örökölt tanulságok, `kubernetes/*` névtérre igazítva)
...
git tag -a "kubernetes/@vX.Y.Z" -m "release: X.Y.Z"
```
