...
- `kubernetes/releases/v*` — kizárólag release tag célra
...
A `cic-kubernetes` egy **domain-repó** — a `cic-primitives` meta-séma rétegére épülve
kubernetes objektumokat lenne hivatva leírni, schema-szinten.
...
**Kubernetes-specifikus domain composition egyelőre nincs megírva** —
...
| Kubernetes-specifikus domain composition | **NOT IMPLEMENTED** | egyetlen saját domain composition sincs még |
| `make validate` zöld | **defined** | Docker-alapú tooling, Vault nélkül is fut |
| signed release pipeline | **defined** | lefutott (lásd git tag-ek), de kubernetes-specifikus tartalom nélkül |
...
base-repo (upstream sablon)
    ...
    └──► cic-primitives
              │  remote: base → git merge base@0.5.0
              └──► cic-kubernetes  (ez a repo)  ·  cic-network, cic-storage, stb. (testvér domain repók)
```
