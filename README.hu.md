...
A `cic-kubernetes` a `cic-primitives` **meta-séma rétegére** épülő domain-repó — kubernetes
objektumokat lenne hivatva leírni, a `cic-primitives` atomic/aggregate
primitíváinak kompozíciójaként.

**Egyelőre nincs kubernetes-specifikus domain composition megírva** — a
`schemas/examples/` alatt csak a `cic-primitives` sablon-demója
(`kubernetes-pod.yaml`) van, ami nem kubernetes-specifikus.
...
| **domain composition** | konkrét kubernetes objektum | — | **még nincs megírva** |
...
| Kubernetes-specifikus domain composition | **NOT IMPLEMENTED** | egyetlen saját domain composition sincs még megírva |
| KubernetesPod sablon-példa | **öröklött, nem kubernetes-specifikus** | `schemas/examples/kubernetes-pod.yaml` — a `cic-primitives` demója |
| Signed release pipeline | **defined** | Vault Transit + ECDSA, a pipeline maga lefutott (lásd git tag-ek), de kubernetes-specifikus tartalom nélkül |
...
```bash
oras pull ghcr.io/centralinfracore/schema/cic-kubernetes:v0.1.2-src2026
```

**curl-lel (ORAS nélkül):**

```bash
REPO="centralinfracore/schema/cic-kubernetes"; TAG="v0.1.2-src2026"; \
